...
UIElement *UIElement::AddElement(UIElement::Ptr anUIElement) {
  Serial.printf("%d UIElement.AddElement(%d)\n", GetID(), anUIElement->GetID());
  auto lock = LvglResourceManager::GetInstance().scopeLock();
...
UIElement::Ptr UIElement::RemoveElement(UIElement *anElementRef) {
  Serial.printf("%d UIElement.RemoveElement(%d)\n", GetID(), anElementRef->GetID());
...
void UIElement::SetWidth(lv_coord_t aWidth) {
  Serial.printf("%d UIElement.SetWidth(%d)\n", GetID(), aWidth);
...
void UIElement::SetHeight(lv_coord_t aHeight) {
  Serial.printf("%d UIElement.SetHeight(%d)\n", GetID(), aHeight);
  LvglResourceManager::GetInstance().AttemptNow(
...
void UIElement::Show() {
  Serial.printf("%d UIElement.Show()\n", GetID());
  if (IsSetVisible()) {
    return;
  }
  {
...
void UIElement::Hide() {
  Serial.printf("%d UIElement.Hide()\n", GetID());
  if (!IsSetVisible()) {
    return;
  }
  {
