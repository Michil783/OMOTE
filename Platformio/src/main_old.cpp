...
lv_obj_t* objBattCharging;
String objBattChargingIcon = LV_SYMBOL_BATTERY_EMPTY;
lv_obj_t* objUSBConnection;
...
void setup() {  
  ...
  lv_obj_set_style_text_font(objBattIcon, &lv_font_montserrat_12, LV_PART_MAIN);

  objBattCharging = lv_label_create(statusbar);
  lv_label_set_text(objBattCharging, LV_SYMBOL_CHARGE);
  lv_obj_align(objBattCharging, LV_ALIGN_RIGHT_MID, -16, 0);
  lv_obj_set_style_text_font(objBattCharging, &lv_font_montserrat_12, LV_PART_MAIN);

  objUSBConnection = lv_label_create(statusbar);
  lv_label_set_text(objUSBConnection, LV_SYMBOL_USB);
  lv_obj_align(objUSBConnection, LV_ALIGN_CENTER, 0, 0);
  lv_obj_set_style_text_font(objUSBConnection, &lv_font_montserrat_12, LV_PART_MAIN);
...
void loop() { 
  ...
  if(millis() - batteryTaskTimer >= 1000){
    ...
    Serial.printf("battery_charging: %s\n", (battery_ischarging ? "TRUE" : "FALSE"));
    // check if volatge is above normal battery level which means, USB is connected
    if( battery_voltage > 4350 )
      lv_label_set_text(objUSBConnection, LV_SYMBOL_USB);
    else
      lv_label_set_text(objUSBConnection, "");
    // Check if battery is charging, fully charged or disconnected
    //if(battery_ischarging || (!battery_ischarging && battery_voltage > 4350)){
    if(battery_ischarging && battery_voltage > 4350){
      //lv_label_set_text(objBattPercentage, "");
      //lv_label_set_text(objBattIcon, LV_SYMBOL_USB);
      lv_label_set_text(objBattCharging, LV_SYMBOL_CHARGE);
      Serial.printf("objBattChargingIcon: %x\n", objBattChargingIcon);
      if(objBattChargingIcon == LV_SYMBOL_BATTERY_EMPTY)
        objBattChargingIcon = LV_SYMBOL_BATTERY_1;
      else if(objBattChargingIcon == LV_SYMBOL_BATTERY_1)
        objBattChargingIcon = LV_SYMBOL_BATTERY_2;
      else if(objBattChargingIcon == LV_SYMBOL_BATTERY_2)
        objBattChargingIcon = LV_SYMBOL_BATTERY_3;
      else if(objBattChargingIcon == LV_SYMBOL_BATTERY_3)
        objBattChargingIcon = LV_SYMBOL_BATTERY_FULL;
      else if(objBattChargingIcon == LV_SYMBOL_BATTERY_FULL)
        objBattChargingIcon = LV_SYMBOL_BATTERY_EMPTY;
      lv_label_set_text(objBattIcon, objBattChargingIcon.c_str());
    }
    else{
      // Update status bar battery indicator
      lv_label_set_text(objBattCharging, "");
